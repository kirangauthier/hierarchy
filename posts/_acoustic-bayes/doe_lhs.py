...
def lhs(n, samples=None, criterion=None, iterations=None, seeded=None):
    """
    ...
    if criterion is not None:
        ...
        H = _lhsclassic(n, samples, seeded)
    ...
    if H is None:
        if criterion.lower() in ('center', 'c'):
            H = _lhscentered(n, samples, seeded)
        elif criterion.lower() in ('maximin', 'm'):
            H = _lhsmaximin(n, samples, iterations, 'maximin', seeded)
        elif criterion.lower() in ('centermaximin', 'cm'):
            H = _lhsmaximin(n, samples, iterations, 'centermaximin', seeded)
        elif criterion.lower() in ('correlate', 'corr'):
            H = _lhscorrelate(n, samples, iterations, seeded)
    
    return H

################################################################################

def _lhsclassic(n, samples, seeded):
...
def _lhscentered(n, samples, seeded):
    # Generate the intervals
    cut = np.linspace(0, 1, samples + 1)    
...
def _lhsmaximin(n, samples, iterations, lhstype, seeded):
    maxdist = 0
    
    # Maximize the minimum distance between points
    for i in range(iterations):
        if lhstype=='maximin':
            Hcandidate = _lhsclassic(n, samples, seeded)
        else:
            Hcandidate = _lhscentered(n, samples, seeded)
...
def _lhscorrelate(n, samples, iterations, seeded):
    mincorr = np.inf
